...
# Extract images from courses.html
...
for course in courses:
    ...
    try:
        req = urllib.request.Request(course['url'], headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req) as response:
            ...
            # Extract paragraphs (crude heuristic for Wix: look for spans with text length > 100)
            # Wix uses a lot of messy spans. A better approach is finding all text content in paragraph blocks
            paragraphs = []
            for match in re.findall(r'<p[^>]*>(.*?)</p>', html_content, re.DOTALL):
                text = clean_html(match)
                if len(text) > 80 and text not in paragraphs and 'rights reserved' not in text and 'Subscribe' not in text:
                    paragraphs.append(text)
            
            para1 = paragraphs[0] if len(paragraphs) > 0 else f"{course['name']} is a transformational professional development program designed for individuals who want to build expertise, emotional adaptability, and the ability to thrive through challenges."
            para2 = paragraphs[1] if len(paragraphs) > 1 else "This course equips participants with practical frameworks, strategies, and development tools required to guide through challenges with confidence and clarity."
            para3 = paragraphs[2] if len(paragraphs) > 2 else "The training combines self-reflection practices, case studies, and guided simulations to ensure participants gain both personal transformation and professional competence."
            
            # Key learning areas - harder to extract consistently from Wix, generating generic ones
            list_items_text = ""
            for item in [
            ...
            img_url = img_url.replace('w_600,h_400', 'w_1920,h_600') # higher res

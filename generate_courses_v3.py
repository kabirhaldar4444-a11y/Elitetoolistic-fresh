...
class WixParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if tag == 'main':
            self.in_main = True
        if self.in_main and tag not in self.ignore_tags:
            self.current_tag = tag
            if tag == 'ul':
                self.in_list = True
                self.extracted_html += "<ul>\n"
            elif tag == 'li':
                self.extracted_html += "  <li>"
            elif tag in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']:
                self.extracted_html += f"<{tag}>"
            elif tag == 'p':
                self.extracted_html += "<p>"
    ...
    def handle_endtag(self, tag):
        if tag == 'main':
            self.in_main = False
        if self.in_main and not self.skip_content:
            if tag == 'ul':
                self.in_list = False
                self.extracted_html += "</ul>\n"
            elif tag == 'li':
    ...
    def handle_data(self, data):
        if self.in_main and not self.skip_content:
            text = data.strip()
            if text:
                # ignore some wix specific empty placeholders
                if text == '\u200b' or text == '​':
                    return
                # Some text in wix is deeply nested inside span. We just append it.
                self.extracted_html += html.escape(text) + " "

def extract_main_content(html_content):
    parser = WixParser()
    parser.feed(html_content)
    
    # Post-process to fix nested tags and clean up
    content = parser.extracted_html
    # Wix often wraps text in spans inside p, so our parser already just outputs <p>text</p> because it ignores spans and just grabs data.
    
    # We want to remove the H1 title since we already have it in the hero section.
    content = re.sub(r'<h1.*?>.*?</h1>\n', '', content, flags=re.IGNORECASE|re.DOTALL)
    
    # The text usually has a lot of <p></p> because of empty spaces
    content = re.sub(r'<p>\s*</p>\n', '', content)
    
    # Also ignore "Join Our Newsletter" and below
    idx = content.find("Join Our Newsletter")
    if idx != -1:
        # Find the preceding <p> or <h2> and cut
        content = content[:idx]
        last_tag_idx = max(content.rfind('<p>'), content.rfind('<h2>'))
        if last_tag_idx != -1:
             content = content[:last_tag_idx]
             
    # Clean up multiple spaces
    content = re.sub(r' +', ' ', content)
    content = content.replace('<p> ', '<p>').replace(' </p>', '</p>').replace('<li> ', '<li>').replace(' </li>', '</li>')
